...
The Studio-only template is lighter than the historical full-stack deployment because it deploys only Studio, Runtime, and the infrastructure required by that path. Recalculate from the Railway project's Usage tab after running representative workloads. External LLM usage is billed by the model provider and is not included in Railway usage.
