...
Enter the credentials configured in your `.env` file. With the Lite strategy on a local stack, the defaults are `admin@alquimia.ai` / `alquimia` — see [Installation](/getting-started/installation#logging-in) for how to change them. After logging in, you'll land on the Studio dashboard.
