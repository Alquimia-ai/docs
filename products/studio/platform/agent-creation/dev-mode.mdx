...
Turn **Dev Mode (advanced)** on with the switch in the **Profile** tab header, top right. It sits at the tab level because it reshapes the whole tab: it unlocks advanced clause editing, reveals the **System Prompt** and **Structured Output** blocks that are hidden in standard mode, and exposes the extra **evaluation strategy** options.

The Profile tab is grouped into three sections, in the order the prompt is built:

| Section | What it holds |
|---------|---------------|
| **Identity** | Description and tags — how the agent is presented and catalogued. |
| **Behavior** | **Custom Rules** (the clauses), then **System Prompt** and **Structured Output** — what those clauses produce. |
| **Execution** | **Reasoning strategy**, step limits, and **Plan Mode** — how the agent runs a request. |
...
<Note>
  A manual system prompt is **remembered**. When you reopen an agent that was saved with one, Studio turns **Dev Mode** and **Manage system prompt manually** back on automatically and loads your prompt, so saving again keeps it. To hand the prompt back to the runtime, clear the checkbox deliberately.
</Note>
